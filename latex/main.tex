...
\begin{center}
    ...
    \textbf{TP.HCM, ngày buồn tháng nhớ năm thương}
    \vspace{\baselineskip}
\end{center}
\end{titlepage}
...
\begin{enumerate}
    \item Đối với hàm Dirac, sử dụng khối \textbf{Step} với hai biến \texttt{impulseDuration} và \texttt{impulseAmplitude} được khai báo trong MATLAB để tạo ra hàm Dirac.
    \vspace{-5mm}
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.5\linewidth]{MATLAB_2.png}
        \vspace{-5mm}
        \caption{Khai báo khối \textbf{Step} đối với hàm Dirac}
    \end{figure}

    \begin{lstlisting}[style=matlabstyle,caption=Khai báo hai biến \texttt{impulseDuration} và \texttt{impulseAmplitude} trong MATLAB để mô phỏng đầu vào là hàm Dirac cho hệ thống]
mdl = "phan_tich_he_thong_ode";
open_system(mdl);
p = [1e-6 0.1 1];
...
out = sim(in,'ShowProgress','off');
    \end{lstlisting}
    \item Đối với hàm nấc đơn vị, sử dụng khối \textbf{Step} được khai báo như hình dưới
    \vspace{-5mm}
    \begin{figure}[ht]
        \centering
...
Vùng màu vàng trong \textbf{Root Locus Editor for LoopTransfer\_C} và  \textbf{Impulse Response} là các vùng mà không thỏa mãn yêu cầu đề bài. Bằng cách kéo thả cặp zero phức của bộ điều khiển và cặp cực của hệ kín (hình vuông màu tím) ở bên cửa sổ \textbf{Root Locus Editor for LoopTransfer\_C} vào vùng màu trắng, nhóm tìm được hàm truyền của bộ điều khiển PID cho góc lắc $\theta$ của con lắc.
\begin{align}
